...
import {addItemToCartAction} from '../../components/store/actions/cartAction'
...
const [error,setError]=useState('');
 ...
 const colors = useMemo(()=>{
   const colorSet = _.uniq(_.map(product?.variats,'color'));
   return colorSet;
 },[product])

 const sizes = useMemo(()=>{
  const sizeSet = _.uniq(_.map(product?.variats,'size'));
  return sizeSet;
},[product])
    ...
    <>
      <div className='flex flex-col md:flex-row px-10'>
              ...
              <div className='flex flex-row md:flex-col justify-center h-full'>
                {
                  product?.productResources?.map((item, index) => (
                    <button key={index} onClick={() => setImage(item?.url)} className='rounded-lg w-fit p-2 mb-2'><img src={item?.url} className='h-[60px] w-[60px] rounded-lg bg-cover bg-center hover:scale-105' alt={'sample' + index} /></button>
                  ))
                }
              </div>
            </div>
        ...
        <div className='w-[60%] px-10'>
          ...
          <div className='mt-2'>
            <SizeFilter sizes={sizes} onChange={(values)=>setSelectedSize(values?.[0])} hideTitle multi={false}/>
          </div>
          <div>
